...
가릅니다. 실측(2026-08, 2석 기준)으로 **18초 → 8초**까지 줄였습니다. 실패해도
어느 단계에서 시간을 썼는지 로그에 남습니다:

```
자동 예매 소요 — 합계 8.2s (화면진입 0.7s · 회차 5.4s · 대기열 0.4s ·
                            인원 0.5s · 좌석맵 0.0s · 좌석고르기 0.4s · 선점 0.9s)
```
...
- **남은 5.4초는 CGV가 회차 목록을 그리는 시간**이라 우리 쪽에서 줄일 수 없습니다.
  다만 **이미 그 화면에 있으면 0초**입니다(딥링크로 새로 열면 6.2초, 재로드해도
  캐시가 듣지 않아 6.2초, SPA 안에서 날짜만 바꾸면 2.6초). 자동 예매를 켠 감시의
  예매 화면을 미리 띄워 두면(프리워밍) 이 6초를 통째로 없앨 수 있습니다 — 아직
  넣지 않았습니다.
