...
RATE_START = 205
# 이 밑으로는 안 내린다 — 감시가 아예 멎으면 의미가 없다.
#
# **바닥값은 평상시 수요보다 커야 한다.** 안 그러면 429 한 번에 영구 기아가 된다:
# 한도가 바닥에 눌리고, 바닥이 수요보다 낮으니 매 사이클 예산이 마르고, 마른
# 상태로는 조용해질 수가 없어 AIMD가 다시 올라가지도 못한다. 실측(2026-09-07)으로
# 정확히 그 상태에 빠져 `예산을 다 썼습니다`가 78번 찍혔다 — 수요 140/분에
# 바닥이 120이었다.
#
# 그래서 수요를 바닥 아래로 내리는 쪽을 함께 했다(폴링 간격 3초 → 5초로 수요
# 140 → 100/분). 수요는 **날짜 수 × (60 / 폴링간격)** 이 지배한다.
#
# **올릴수록 429가 나도 덜 물러선다는 뜻이다.** 바닥이 높으면 CGV가 거절해도
# 우리는 그 아래로 안 내려가므로, 거절이 이어지면 그만큼 계속 거절당한다.
RATE_FLOOR = 120
# 이 위로는 안 올린다. **1800으로 올렸다가 300으로 내렸다.**
#
# 1800은 "AIMD가 진짜 한계를 찾게 하자"는 생각이었는데, 찾는 과정 자체가 비쌌다.
# 실측(2026-09-07)으로 429를 맞은 한도가 이렇게 나왔다:
#
#   715 → 665 → 357 → 332 → 166 → 130
#
# 한도가 665까지 올라간 뒤 한 초에 429를 세 번 맞고 665 → 332 → 166 → 120으로
# 곤두박질쳤다. 올라가는 데 몇십 분, 떨어지는 데 1초다 — 그 비대칭 때문에 높은
# 천장은 "가끔 빠르고 대체로 바닥"이라는 결과를 낳는다.
#
# 그래서 관측된 안전지대 안에 둔다: 205/분은 무사했고 332부터 거절당했다.
# 300은 그 사이이고, AIMD는 이 좁은 띠 안에서만 진동한다.
RATE_CEILING = 300
